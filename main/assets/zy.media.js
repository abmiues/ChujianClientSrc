...
(function() {
	...
	zyMedia.MediaPlayer.prototype = {
		...
		showControls: function() {
			...
			if(t.options.backBtn)
            {
            	t.back_btn.style.top='6px'
            }
		...
		buildContainer: function() {
			...
			t.container.innerHTML = '<div class="zy_wrap"></div><div class="zy_controls"></div>' +
				(t.options.mediaTitle ? '<div class="zy_title">' + t.options.mediaTitle + '</div>' : '')+'<div class="zy_back_btn"></div>';

			t.title = t.container.querySelector('.zy_title');
			t.back_btn=t.container.querySelector('.zy_back_btn');
	...
	window.zymedia = function(selector, options) {
		if (typeof selector === 'string') {
			[].forEach.call(document.querySelectorAll(selector), function(el) {
				zyMedias=new zyMedia.MediaPlayer(el, options)
			});
		} else {
			zyMedias=new zyMedia.MediaPlayer(selector, options)
		}
	}
	window.fullScreen=function() {
		zyMedias.media.play();
		zyMedias.enterFullScreen();
	}
